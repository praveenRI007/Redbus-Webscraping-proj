...
st.set_page_config(page_title="Red Bus Dashboard App")

st.sidebar.header("Red Bus Dashboard App")
...
st.markdown(
    """
    The Redbus Data Scraping and Filtering project automates the extraction of bus travel data from the Redbus website using Selenium, storing it in an SQLite database, and visualizing it through a Streamlit dashboard. The scraped data includes routes, schedules, prices, ratings, and seat availability, providing insights for various business applications like travel aggregators, market analysis, and competitor comparisons. The interactive Streamlit app enables users to filter and analyze data based on parameters such as bus type, price, and availability, facilitating data-driven decision-making and enhancing operational efficiency in the transportation industry.
"""
)
